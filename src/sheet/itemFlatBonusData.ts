...
import { itemKey, wornAcBonusOf, type ItemRef } from '../inventory/inventoryData'
 ...
 *
 * D43: a row absent from items.json is announced only when it is ATTUNED. An
		...
		if (!ref) {
			...
			for (const target of FLAT_BONUS_TARGETS) {
				grants.push({
					...
					target,
					amount: 0,
					unresolvedReason: `attuned but not found in the item data (${item.source}) — any flat bonus it grants is not counted`,
				})
			}
			continue
		}
