...
Last updated: 2026-07-29 (species skill proficiencies picker added to the wizard)
...
- Storage layer — src/storage/, localStorage key `familliar:characters`, schema version 5. A saved character carries classSkills, speciesSkills, masteries, fightingStyle, (via CharacterClass.subclass) subclass, the background's two fixed skill proficiencies (CharacterBackground.skillProficiencies), and optionalFeatureChoices — an array of `{ featureType, choices }`, so picks from more than one subclass progression could coexist without ambiguity. A version-4 save is rejected, not migrated (see docs/QUESTIONS.md, "Migrace uložených postav").
